...
func TestInt256(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_int256`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_int256 (
				int256 Int256,
				int256_nullable Nullable(Int256),
				int256_array Array(Int256),
				int256_array_nullable Array(Nullable(Int256))
			) Engine=Memory`)

	require.NoError(t, err)
	require.Nil(t, res)

	col := column.NewInt256(false)

	colArrayValues := column.NewInt256(false)
	colArray := column.NewArray(colArrayValues)

	colArrayValuesNil := column.NewInt256(true)
	colArrayNil := column.NewArray(colArrayValuesNil)

	colNil := column.NewInt256(true)
	...
	rows := 10
	for i := 1; i <= rows; i++ {
		val := make([]byte, column.Int256Size)
		val[0] = byte(i)
		val2 := make([]byte, column.Int256Size)
		val2[0] = byte(i + 1)
	...
	err = conn.Insert(context.Background(), `INSERT INTO
		test_int256 (int256,int256_nullable,int256_array,int256_array_nullable)
	VALUES`,
		col,
		colNil,
		colArray,
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		int256,int256_nullable,int256_array,int256_array_nullable
	FROM test_int256`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())

	colRead := column.NewInt256(false)
	colNilRead := column.NewInt256(true)
	colArrayReadData := column.NewInt256(false)
	colArrayRead := column.NewArray(colArrayReadData)
	colArrayReadDataNil := column.NewInt256(true)
	...
	// example one by one
	selectStmt, err = conn.Select(context.Background(), `SELECT
		int256,int256_nullable,int256_array,int256_array_nullable FROM
	test_int256`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
