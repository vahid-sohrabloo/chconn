...
func TestUint256(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_uint256`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_uint256 (
				uint256 UInt256,
				uint256_nullable Nullable(UInt256),
				uint256_array Array(UInt256),
				uint256_array_nullable Array(Nullable(UInt256))
			) Engine=Memory`)

	require.NoError(t, err)
	require.Nil(t, res)

	col := column.NewUint256(false)

	colArrayValues := column.NewUint256(false)
	colArray := column.NewArray(colArrayValues)

	colArrayValuesNil := column.NewUint256(true)
	colArrayNil := column.NewArray(colArrayValuesNil)

	colNil := column.NewUint256(true)
	...
	rows := 10
	for i := 1; i <= rows; i++ {
		val := make([]byte, column.Uint256Size)
		val[0] = byte(i)
		val2 := make([]byte, column.Uint256Size)
		val2[0] = byte(i + 1)
	...
	insertstmt, err := conn.Insert(context.Background(), `INSERT INTO
		test_uint256 (uint256,uint256_nullable,uint256_array,uint256_array_nullable)
	VALUES`)
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		uint256,uint256_nullable,uint256_array,uint256_array_nullable
	FROM test_uint256`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())

	colRead := column.NewUint256(false)
	colNilRead := column.NewUint256(true)
	colArrayReadData := column.NewUint256(false)
	colArrayRead := column.NewArray(colArrayReadData)
	colArrayReadDataNil := column.NewUint256(true)
	...
	// example one by one
	selectStmt, err = conn.Select(context.Background(), `SELECT
		uint256,uint256_nullable,uint256_array,uint256_array_nullable FROM
	test_uint256`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
