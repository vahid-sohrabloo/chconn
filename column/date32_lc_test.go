...
func TestDate32LC(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_date32`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_date32 (
				date32_lc LowCardinality(Date32),
				date32_lc_nullable LowCardinality(Nullable(Date32)),
				date32_lc_array Array(LowCardinality(Date32)),
				date32_lc_array_nullable Array(LowCardinality(Nullable(Date32)))
	...
	colNil := column.NewDate32(true)
	colNilLC := column.NewLC(colNil)

	colArrayValues := column.NewDate32(false)
	collArrayLC := column.NewLC(colArrayValues)
	colArray := column.NewArray(collArrayLC)

	colArrayValuesNil := column.NewDate32(true)
	...
	err = conn.Insert(context.Background(), `INSERT INTO
		test_lc_date32(date32_lc,date32_lc_nullable,date32_lc_array,date32_lc_array_nullable)
	VALUES`,
		colLC,
		colNilLC,
		colArray,
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT date32_lc,
		date32_lc_nullable,date32_lc_array,date32_lc_array_nullable FROM
	test_lc_date32`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())

	colRead := column.NewDate32(false)
	colLCRead := column.NewLC(colRead)

	colNilRead := column.NewDate32(true)
	colNilLCRead := column.NewLC(colNilRead)

	colArrayReadData := column.NewDate32(false)
	colArrayLCRead := column.NewLC(colArrayReadData)
	colArrayRead := column.NewArray(colArrayLCRead)

	colArrayReadDataNil := column.NewDate32(true)
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		date32_lc,date32_lc_nullable FROM
	test_lc_date32`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
