...
func TestInt32(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_int32`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_int32 (
				int32 Int32,
				int32_nullable Nullable(Int32),
				int32_array Array(Int32),
				int32_array_nullable Array(Nullable(Int32))
			) Engine=Memory`)
	...
	for insertN := 0; insertN < 2; insertN++ {
		...
		err = conn.Insert(context.Background(), `INSERT INTO
			test_int32 (int32,int32_nullable,int32_array,int32_array_nullable)
		VALUES`,
			col,
			colNil,
			colArray,
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		int32,int32_nullable,int32_array,int32_array_nullable
	FROM test_int32`)
	require.NoError(t, err)
	...
	for selectStmt.Next() {
		...
		for colArrayRead.Next() {
			arr := make([]int32, colArrayRead.Value())
			colArrayReadData.Fill(arr)
			colArrayData = append(colArrayData, arr)
		}
		...
		for colArrayReadNil.Next() {
			arr := make([]*int32, colArrayReadNil.Value())
			colArrayReadDataNil.FillP(arr)
			colArrayDataNil = append(colArrayDataNil, arr)
		}
	}
