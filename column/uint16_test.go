...
func TestUint16(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_uint16`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_uint16 (
				uint16 UInt16,
				uint16_nullable Nullable(UInt16),
				uint16_array Array(UInt16),
				uint16_array_nullable Array(Nullable(UInt16))
			) Engine=Memory`)
	...
	for insertN := 0; insertN < 2; insertN++ {
		...
		err = conn.Insert(context.Background(), `INSERT INTO
			test_uint16 (uint16,uint16_nullable,uint16_array,uint16_array_nullable)
		VALUES`,
			col,
			colNil,
			colArray,
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		uint16,uint16_nullable,uint16_array,uint16_array_nullable
	FROM test_uint16`)
	require.NoError(t, err)
	...
	for selectStmt.Next() {
		...
		for colArrayRead.Next() {
			arr := make([]uint16, colArrayRead.Value())
			colArrayReadData.Fill(arr)
			colArrayData = append(colArrayData, arr)
		}
		...
		for colArrayReadNil.Next() {
			arr := make([]*uint16, colArrayReadNil.Value())
			colArrayReadDataNil.FillP(arr)
			colArrayDataNil = append(colArrayDataNil, arr)
		}
	}
...
func TestUint16FillEmpty(t *testing.T) {
	t.Parallel()
	col := column.NewUint16(false)
	col.Fill([]uint16{})
	col.Fill(nil)
