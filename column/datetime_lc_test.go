...
func TestDateTimeLC(t *testing.T) {
	t.Parallel()
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_datetime`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_datetime (
				datetime_lc LowCardinality(DateTime),
				datetime_lc_nullable LowCardinality(Nullable(DateTime)),
				datetime_lc_array Array(LowCardinality(DateTime)),
				datetime_lc_array_nullable Array(LowCardinality(Nullable(DateTime)))
	...
	err = conn.Insert(context.Background(), `INSERT INTO
		test_lc_datetime(datetime_lc,datetime_lc_nullable,datetime_lc_array,datetime_lc_array_nullable)
	VALUES`,
		colLC,
		colNilLC,
		colArray,
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT datetime_lc,
		datetime_lc_nullable,datetime_lc_array,datetime_lc_array_nullable FROM
	test_lc_datetime`)
	require.NoError(t, err)
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		datetime_lc,datetime_lc_nullable FROM
	test_lc_datetime`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
