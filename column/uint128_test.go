...
func TestUint128(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_uint128`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_uint128 (
				uint128 UInt128,
				uint128_nullable Nullable(UInt128),
				uint128_array Array(UInt128),
				uint128_array_nullable Array(Nullable(UInt128))
			) Engine=Memory`)

	require.NoError(t, err)
	require.Nil(t, res)

	col := column.NewUint128(false)

	colArrayValues := column.NewUint128(false)
	colArray := column.NewArray(colArrayValues)

	colArrayValuesNil := column.NewUint128(true)
	colArrayNil := column.NewArray(colArrayValuesNil)

	colNil := column.NewUint128(true)
	...
	rows := 10
	for i := 1; i <= rows; i++ {
		val := make([]byte, column.Uint128Size)
		val[0] = byte(i)
		val2 := make([]byte, column.Uint128Size)
		val2[0] = byte(i + 1)
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		uint128,uint128_nullable,uint128_array,uint128_array_nullable
	FROM test_uint128`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())

	colRead := column.NewUint128(false)
	colNilRead := column.NewUint128(true)
	colArrayReadData := column.NewUint128(false)
	colArrayRead := column.NewArray(colArrayReadData)
	colArrayReadDataNil := column.NewUint128(true)
	...
	// example one by one
	selectStmt, err = conn.Select(context.Background(), `SELECT
		uint128,uint128_nullable,uint128_array,uint128_array_nullable FROM
	test_uint128`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
