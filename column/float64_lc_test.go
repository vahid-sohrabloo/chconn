...
func TestFloat64LC(t *testing.T) {
	t.Parallel()
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_float64`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_float64 (
				float64_lc LowCardinality(Float64),
				float64_lc_nullable LowCardinality(Nullable(Float64)),
				float64_lc_array Array(LowCardinality(Float64)),
				float64_lc_array_nullable Array(LowCardinality(Nullable(Float64)))
	...
	err = conn.Insert(context.Background(), `INSERT INTO
		test_lc_float64(float64_lc,float64_lc_nullable,float64_lc_array,float64_lc_array_nullable)
	VALUES`,
		colLC,
		colNilLC,
		colArray,
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT float64_lc,
		float64_lc_nullable,float64_lc_array,float64_lc_array_nullable FROM
	test_lc_float64`)
	require.NoError(t, err)
	...
	var colDataDict []float64
	var colDataKeys []int
	var colData []float64

	var colNilDataDict []float64
	var colNilDataKeys []int
	var colNilData []*float64

	var colArrayDataDict []float64
	var colArrayData [][]float64

	var colArrayDataDictNil []float64
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		float64_lc,float64_lc_nullable FROM
	test_lc_float64`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
