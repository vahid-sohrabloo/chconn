...
func TestInt32LC(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_int32`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_int32 (
				int32_lc LowCardinality(Int32),
				int32_lc_nullable LowCardinality(Nullable(Int32)),
				int32_lc_array Array(LowCardinality(Int32)),
				int32_lc_array_nullable Array(LowCardinality(Nullable(Int32)))
	...
	colNil := column.NewInt32(true)
	colNilLC := column.NewLC(colNil)

	colArrayValues := column.NewInt32(false)
	collArrayLC := column.NewLC(colArrayValues)
	colArray := column.NewArray(collArrayLC)

	colArrayValuesNil := column.NewInt32(true)
	collArrayLCNil := column.NewLC(colArrayValuesNil)
	colArrayNil := column.NewArray(collArrayLCNil)

	var colInsert []int32
	var colInsertArray [][]int32
	var colInsertArrayNil [][]*int32
	var colNilInsert []*int32

	rows := 10
	for i := 1; i <= rows; i++ {
		val := int32(i * -4)
		valArray := []int32{val, int32(i*-4) + 1}
		valArrayNil := []*int32{&val, nil}
	...
	insertstmt, err := conn.Insert(context.Background(), `INSERT INTO
		test_lc_int32(int32_lc,int32_lc_nullable,int32_lc_array,int32_lc_array_nullable)
	VALUES`)
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT int32_lc,
		int32_lc_nullable,int32_lc_array,int32_lc_array_nullable FROM
	test_lc_int32`)
	require.NoError(t, err)
	...
	colNilRead := column.NewInt32(true)
	colNilLCRead := column.NewLC(colNilRead)

	colArrayReadData := column.NewInt32(false)
	colArrayLCRead := column.NewLC(colArrayReadData)
	colArrayRead := column.NewArray(colArrayLCRead)

	colArrayReadDataNil := column.NewInt32(true)
	colArrayLCReadNil := column.NewLC(colArrayReadDataNil)
	colArrayReadNil := column.NewArray(colArrayLCReadNil)

	var colDataDict []int32
	var colDataKeys []int
	var colData []int32

	var colNilDataDict []int32
	var colNilDataKeys []int
	var colNilData []*int32

	var colArrayDataDict []int32
	var colArrayData [][]int32

	var colArrayDataDictNil []int32
	var colArrayDataNil [][]*int32
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		int32_lc,int32_lc_nullable FROM
	test_lc_int32`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
