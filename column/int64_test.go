...
func TestInt64(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_int64`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_int64 (
				int64 Int64,
				int64_nullable Nullable(Int64),
				int64_array Array(Int64),
				int64_array_nullable Array(Nullable(Int64))
			) Engine=Memory`)
	...
	for insertN := 0; insertN < 2; insertN++ {
		...
		err = conn.Insert(context.Background(), `INSERT INTO
			test_int64 (int64,int64_nullable,int64_array,int64_array_nullable)
		VALUES`,
			col,
			colNil,
			colArray,
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		int64,int64_nullable,int64_array,int64_array_nullable
	FROM test_int64`)
	require.NoError(t, err)
	...
	for selectStmt.Next() {
		...
		for colArrayRead.Next() {
			arr := make([]int64, colArrayRead.Value())
			colArrayReadData.Fill(arr)
			colArrayData = append(colArrayData, arr)
		}
		...
		for colArrayReadNil.Next() {
			arr := make([]*int64, colArrayReadNil.Value())
			colArrayReadDataNil.FillP(arr)
			colArrayDataNil = append(colArrayDataNil, arr)
		}
	}
...
func TestInt64FillEmpty(t *testing.T) {
	t.Parallel()
	col := column.NewInt64(false)
	col.Fill([]int64{})
	col.Fill(nil)
