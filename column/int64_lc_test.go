...
func TestInt64LC(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_int64`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_int64 (
				int64_lc LowCardinality(Int64),
				int64_lc_nullable LowCardinality(Nullable(Int64)),
				int64_lc_array Array(LowCardinality(Int64)),
				int64_lc_array_nullable Array(LowCardinality(Nullable(Int64)))
	...
	colNil := column.NewInt64(true)
	colNilLC := column.NewLC(colNil)

	colArrayValues := column.NewInt64(false)
	collArrayLC := column.NewLC(colArrayValues)
	colArray := column.NewArray(collArrayLC)

	colArrayValuesNil := column.NewInt64(true)
	collArrayLCNil := column.NewLC(colArrayValuesNil)
	colArrayNil := column.NewArray(collArrayLCNil)

	var colInsert []int64
	var colInsertArray [][]int64
	var colInsertArrayNil [][]*int64
	var colNilInsert []*int64

	rows := 10
	for i := 1; i <= rows; i++ {
		val := int64(i * -8)
		valArray := []int64{val, int64(i*-8) + 1}
		valArrayNil := []*int64{&val, nil}
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT int64_lc,
		int64_lc_nullable,int64_lc_array,int64_lc_array_nullable FROM
	test_lc_int64`)
	require.NoError(t, err)
	...
	var colNilDataDict []int64
	var colNilDataKeys []int
	var colNilData []*int64

	var colArrayDataDict []int64
	var colArrayData [][]int64

	var colArrayDataDictNil []int64
	var colArrayDataNil [][]*int64
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		int64_lc,int64_lc_nullable FROM
	test_lc_int64`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
