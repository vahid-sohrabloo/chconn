...
func TestDate32(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_date32`)
	require.NoError(t, err)
	require.Nil(t, res)

	res, err = conn.Exec(context.Background(), `CREATE TABLE test_date32 (
				date32 Date32,
				date32_nullable Nullable(Date32),
				date32_array Array(Date32),
				date32_array_nullable Array(Nullable(Date32))
			) Engine=Memory`)
	...
	insertstmt, err := conn.Insert(context.Background(), `INSERT INTO
		test_date32 (date32,date32_nullable,date32_array,date32_array_nullable)
	VALUES`)
	...
	selectStmt, err := conn.Select(context.Background(), `SELECT
		date32,date32_nullable,date32_array,date32_array_nullable
	FROM test_date32`)
	require.NoError(t, err)
