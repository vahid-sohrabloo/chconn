...
func TestInt16LC(t *testing.T) {
	...
	res, err := conn.Exec(context.Background(), `DROP TABLE IF EXISTS test_lc_int16`)
	require.NoError(t, err)
	require.Nil(t, res)
	settings := setting.NewSettings()
	settings.AllowSuspiciousLowCardinalityTypes(true)
	res, err = conn.ExecWithSetting(context.Background(), `CREATE TABLE test_lc_int16 (
				int16_lc LowCardinality(Int16),
				int16_lc_nullable LowCardinality(Nullable(Int16)),
				int16_lc_array Array(LowCardinality(Int16)),
				int16_lc_array_nullable Array(LowCardinality(Nullable(Int16)))
	...
	err = conn.Insert(context.Background(), `INSERT INTO
		test_lc_int16(int16_lc,int16_lc_nullable,int16_lc_array,int16_lc_array_nullable)
	VALUES`,
		colLC,
		colNilLC,
		colArray,
	...
	// example read all
	selectStmt, err := conn.Select(context.Background(), `SELECT int16_lc,
		int16_lc_nullable,int16_lc_array,int16_lc_array_nullable FROM
	test_lc_int16`)
	require.NoError(t, err)
	...
	var colDataDict []int16
	var colDataKeys []int
	var colData []int16

	var colNilDataDict []int16
	var colNilDataKeys []int
	var colNilData []*int16

	var colArrayDataDict []int16
	var colArrayData [][]int16

	var colArrayDataDictNil []int16
	...
	selectStmt, err = conn.Select(context.Background(), `SELECT
		int16_lc,int16_lc_nullable FROM
	test_lc_int16`)
	require.NoError(t, err)
	require.True(t, conn.IsBusy())
