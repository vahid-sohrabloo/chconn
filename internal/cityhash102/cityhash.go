...
func cityMurmur(s []byte, length uint32, seed Uint128) Uint128 {
	var c, d uint64
	a, b := seed.Lower64(), seed.Higher64()
	l := int32(length) - 16
...
func CityHash128WithSeed(s []byte, length uint32, seed Uint128) Uint128 {
	...
	var v, w Uint128
	var pos uint32
	x, y := seed.Lower64(), seed.Higher64()
	z := uint64(length) * k1
	t := s
	...
	for {
		...
		w = weakHashLen32WithSeeds3(s[32:], z+w.Higher64(), y)
		swap64(&z, &x)
		s = s[64:]
		pos += 64
	...
	for tailDone = 0; tailDone < length; {
		...
		v = weakHashLen32WithSeeds3(t[pos+length-tailDone:], v.Lower64(), v.Higher64())
	}
