...
func TestConnectError(t *testing.T) {
	...
	assert.EqualError(t, err, "failed to connect to `host=127.0.0.1 user=invalid username database=default`: server error ( DB::Exception (516): invalid username: Authentication failed: password is incorrect or there is no user with such name)")
	assert.EqualError(t, errors.Unwrap(err), " DB::Exception (516): invalid username: Authentication failed: password is incorrect or there is no user with such name")
	assert.Nil(t, conn)

	conn, err = Connect(context.Background(), "host>0")
	assert.EqualError(t, err, "cannot parse `host>0`: failed to parse as DSN (invalid dsn)")
	assert.Nil(t, conn)

	conn, err = Connect(context.Background(), "host=invalid_host")
	assert.EqualError(t, err, "failed to connect to `host=invalid_host user=default database=default`: hostname resolving error (lookup invalid_host: no such host)")
	assert.Nil(t, conn)
	...
	conn, err = ConnectConfig(context.Background(), config)
	assert.EqualError(t, err, "failed to connect to `host=127.0.0.1 user=default database=default`: dial error (dial tcp 127.0.0.1:63666: connect: connection refused)")
	assert.Nil(t, conn)
	...
	assert.EqualError(t, err, "failed to connect to `host=127.0.0.1 user=default database=default`: AfterConnect error (afterConnect err)")
...
func TestException(t *testing.T) {
	...
	require.NoError(t, conn.Ping(context.Background()))
	res, err := conn.Exec(context.Background(), `invalid query`, nil)
...
func TestExecError(t *testing.T) {
	...
	c.(*conn).status = connStatusUninitialized
	res, err := c.Exec(context.Background(), "SET enable_http_compression=1", nil)
	require.Nil(t, res)
	require.EqualError(t, c.(*conn).lock(), "conn uninitialized")
...
func TestRecivePackError(t *testing.T) {
	...
	c, err := ConnectConfig(context.Background(), config)
	require.NoError(t, err)
	res, err := c.Exec(context.Background(), `SELECT * FROM system.numbers limit 1`, nil)
